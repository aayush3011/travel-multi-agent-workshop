...
def create_thread(
    ...
    """
    Create a new conversation thread with proper initialization.
    ...
    Returns:
        Dictionary with thread details including threadId
    """
    logger.info(f"🆕 Creating thread for user: {user_id}")
    thread = create_thread_record(user_id, tenant_id, title, activeAgent)
    return {
...
def get_thread_context(
    ...
    messages = get_thread_messages(thread_id, tenant_id, user_id)
    thread_info = get_thread_by_id(thread_id, tenant_id, user_id)
    ...
    if include_summaries:
        summaries = get_thread_summaries(thread_id, tenant_id, user_id)
...
def append_turn(
    ...
    """
    Atomically store a message and update thread metadata.
    ...
    """
    logger.info(f"💬 Appending {role} message to thread: {thread_id}")
...
def mark_span_summarized(
    ...
    """
    logger.info(f"📝 Creating summary for thread: {thread_id}")
...
def get_summarizable_span(
    ...
    """
    logger.info(f"📊 Finding summarizable span for thread: {thread_id}")
...
def update_trip(
    ...
    # Save to Cosmos DB
    from python.src.app.services.azure_cosmos_db import trips_container
    if trips_container:
...
def search_user_threads(
    ...
    from python.src.app.services.azure_cosmos_db import messages_container, summaries_container
