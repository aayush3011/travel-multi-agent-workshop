...
from python.src.app.services.azure_cosmos_db import (
    ...
    threads_container, patch_active_agent,
    update_thread_container, append_message
)
...
async def setup_agents():
    ...
    # Orchestrator: Thread management + all transfer tools
    orchestrator_tools = filter_tools_by_prefix(all_tools, [
        "create_thread", "get_thread_context", "append_turn",
    ...
    # Handles TWO flows:
    # 1. WARM START: User already discussed hotels/restaurants → use get_thread_context to extract them
    ...
    itinerary_generator_tools = filter_tools_by_prefix(all_tools, [
        ...
        "get_thread_context",  # Read conversation to extract recommended places (WARM START)
    ...
    summarizer_tools = filter_tools_by_prefix(all_tools, [
        "get_summarizable_span", "mark_span_summarized", "get_thread_context",
...
async def call_orchestrator_agent(state: MessagesState, config) -> Command[Literal["orchestrator", "hotel", "activity", "dining", "itinerary_generator", "summarizer", "human"]]:
    ...
    logger.info(f"🎯 Calling orchestrator agent with Thread ID: {thread_id}")
    ...
    try:
        thread_doc = threads_container.read_item(
            item=thread_id, 
            partition_key=[tenant_id, user_id, thread_id]
        )
        activeAgent = thread_doc.get('activeAgent', 'unknown')
    except Exception as e:
        logger.debug(f"No active agent found: {e}")
        activeAgent = None
    
    # Initialize thread if needed (for local testing)
    if activeAgent is None:
        update_thread_container({
            "id": thread_id,
            "threadId": thread_id,
            "tenantId": tenant_id,
            "userId": user_id,
...
async def call_summarizer_agent(state: MessagesState, config) -> Command[Literal["summarizer", "orchestrator", "human"]]:
    ...
    # Patch active agent in database
    if local_interactive_mode:
        patch_active_agent(tenant_id or "cli-test", user_id or "cli-test", thread_id, "summarizer")
...
def get_active_agent(state: MessagesState, config) -> str:
    ...
    if not activeAgent:
        try:
            ...
            activeAgent = thread_doc.get('activeAgent', 'unknown')
            logger.info(f"Active agent from DB fallback: {activeAgent}")
        except Exception as e:
            logger.error(f"Error retrieving active agent from DB: {e}")
            activeAgent = "unknown"
        # If activeAgent is unknown or orchestrator, return to human (wait for user input)
        # This prevents orchestrator from calling itself in a loop
...
def build_agent_graph():
    ...
    # Itinerary Generator routing - can only return to orchestrator
    builder.add_conditional_edges(
        "itinerary_generator",
        get_active_agent,
        {
            "orchestrator": "orchestrator",
            "itinerary_generator": "itinerary_generator",  # Can stay
        }
    )
    ...
    logger.info("   Summarizer → Orchestrator only (auto-triggered every 10 turns)")
