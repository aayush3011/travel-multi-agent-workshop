...
from python.src.app.services.azure_open_ai import model
from python.src.app.services.azure_cosmos_db import (
    threads_container, messages_container, trips_container,
    memories_container, places_container, get_checkpoint_saver,
    create_thread_record, get_thread_by_id,
    append_message, get_thread_messages,
    get_trip, query_memories, query_places,
    patch_active_agent, update_thread_activity
)
from python.src.app.travel_agents import setup_agents, build_agent_graph, cleanup_persistent_session
...
def get_service_status():
    return {
        ...
        "cosmos_db": "connected" if threads_container else "disconnected"
    }
...
@app.post(
    "/tenant/{tenantId}/user/{userId}/threads",
    tags=[THREAD_TAG],
    summary="Create New Thread",
    description="Create a new conversation thread for the user",
    response_model=Thread,
    status_code=201
)
def create_chat_thread(tenantId: str, userId: str, activeAgent: str, title: str = None):
    """
    ...
    Returns:
        Thread object with threadId and metadata
    """
    try:
        thread = create_thread_record(userId, tenantId, activeAgent, title)
...
def get_user_threads(tenantId: str, userId: str):
    """
    ...
    try:
        ...
        return [Thread(**item) for item in items]
...
@app.get(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/messages",
    tags=[THREAD_TAG],
    summary="Get Thread Messages",
    description="Retrieve conversation history for a specific thread",
    response_model=List[MessageModel]
)
def get_thread_messages_endpoint(tenantId: str, userId: str, threadId: str):
    """
    ...
    """
    try:
        messages = get_thread_messages(threadId, tenantId, userId)
...
@app.post(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/rename",
    tags=[THREAD_TAG],
    summary="Rename Thread",
    description="Update the title of a conversation thread",
    response_model=Thread
)
def rename_thread(tenantId: str, userId: str, threadId: str, newThreadName: str):
    """
    ...
    Args:
        ...
        userId: User identifier
        threadId: Thread identifier
        newThreadName: New title for the thread
    
    Returns:
        Updated Thread object
    """
    try:
        thread = get_thread_by_id(threadId, tenantId, userId)
        if not thread:
            raise HTTPException(status_code=404, detail="Thread not found")
        
        thread["title"] = newThreadName
...
@app.delete(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}",
    tags=[THREAD_TAG],
    summary="Delete Thread",
    description="Delete a conversation thread and all associated data",
    status_code=200
)
def delete_thread(tenantId: str, userId: str, threadId: str, background_tasks: BackgroundTasks):
    """
    Delete a conversation thread and all related data (messages, checkpoints).
    ...
    try:
        ...
        if threads_container:
            ...
            threads_container.delete_item(item=threadId, partition_key=partition_key)
        ...
        def delete_checkpoints():
            try:
                if _checkpointer and hasattr(_checkpointer, 'container'):
                    query = "SELECT c.id, c.partition_key FROM c WHERE CONTAINS(c.partition_key, @threadId)"
        ...
        return {"message": "Thread deleted successfully", "threadId": threadId}
    except Exception as e:
...
def store_debug_log(threadId: str, tenantId: str, userId: str, response_data: List[Dict]) -> str:
    """Store detailed debug log information"""
    ...
    # Note: In production, you'd store this in a debug_logs container
    # For now, we'll just return the ID
    logger.debug(f"Debug log created: {debug_log_id} for thread {threadId}")
...
def extract_relevant_messages(
    ...
    # Update active agent in thread
    patch_active_agent(tenantId, userId, threadId, last_agent_name)
...
def process_messages_background(messages: List[MessageModel], userId: str, tenantId: str, threadId: str):
    ...
    try:
        ...
        update_thread_activity(threadId, tenantId, userId)
        
        logger.info(f"✅ Stored {len(messages)} messages for thread {threadId}")
    except Exception as e:
        logger.error(f"Error storing messages: {e}")


@app.post(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/completion",
    tags=[CHAT_TAG],
...
async def get_chat_completion(
    ...
    try:
        ...
        # Store debug log
        debug_log_id = store_debug_log(threadId, tenantId, userId, response_data)
        ...
        thread = get_thread_by_id(threadId, tenantId, userId)
        if thread:
            active_agent = thread.get("activeAgent", "orchestrator")
        ...
        background_tasks.add_task(
            ...
            messages, userId, tenantId, threadId
        )
...
@app.post(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/summarize-name",
    tags=[CHAT_TAG],
    summary="Auto-Generate Thread Title",
    description="Generate a descriptive thread title based on conversation content",
    response_model=str
)
async def summarize_thread_name(
    tenantId: str,
    userId: str,
    threadId: str,
    request_body: str = Body(..., media_type="application/json")
    ...
    """
    Generate a concise thread title from conversation text.
...
def search_places(search_request: PlaceSearchRequest):
    ...
    try:
        ...
        from python.src.app.services.azure_open_ai import generate_embedding
...
@app.get(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/completiondetails/{debugLogId}",
    tags=[DEBUG_TAG],
    summary="Get Debug Information",
    description="Retrieve detailed debug information for a chat completion",
    response_model=Dict[str, Any]
)
def get_completion_details(tenantId: str, userId: str, threadId: str, debugLogId: str):
    """
    ...
    """
    # Note: In production, you'd query a debug_logs container
...
@app.post(
    "/tenant/{tenantId}/user/{userId}/threads/{threadId}/message/{messageId}/rate",
    tags=[DEBUG_TAG],
    summary="Rate Message",
    description="Rate an AI response with thumbs up/down",
    response_model=MessageModel
)
def rate_message(tenantId: str, userId: str, threadId: str, messageId: str, rating: bool):
    """
    Rate an AI response.
